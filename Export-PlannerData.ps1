...
function Get-PlansByGroupIds {
    ...
    foreach ($groupId in $GroupIds) {
        $groupDisplayName = $groupId   # Fallback falls Gruppenname-Abruf fehlschlägt
        ...
        try {
            ...
            $errMsg = $_.ToString()
            if ($errMsg -match 'permission|403|Forbidden|Unauthorized|You do not have') {
                Write-PlannerLog "Zugriff verweigert für Gruppe '$groupDisplayName' ($groupId) - Pläne dieser Gruppe werden nicht exportiert." "ERROR"
                Write-PlannerLog "  Hinweis: Das angemeldete Konto ist möglicherweise kein Mitglied dieser Gruppe." "WARN"
            }
            else {
                Write-PlannerLog "Fehler beim Laden der Pläne für Gruppe '$groupDisplayName' ($groupId): $_" "ERROR"
            }
        }
    }

    # Komma-Operator verhindert Pipeline-Unrolling leerer Arrays (PowerShell-Eigenheit)
    return ,$plans
}
...
function Export-PlanDetails {
    ...
    try {
        ...
        foreach ($task in $allTasks) {
            $counter++
            Write-Progress -Activity "Lade Task-Details für '$($Plan.title)'" -Status "Task $counter von $($allTasks.Count)" -PercentComplete (($counter / [Math]::Max(1, $allTasks.Count)) * 100)
    ...
    if ($script:exportedFileNames.ContainsKey($planFileName)) {
        $firstGroup = $script:exportedFileNames[$planFileName]
        Write-PlannerLog "  WARNUNG: '$planFileName.json' wird überschrieben - zwei Pläne mit gleichem Namen ('$($Plan.groupDisplayName)' und '$firstGroup')" "WARN"
    }
    $script:exportedFileNames[$planFileName] = $Plan.groupDisplayName
...
elseif ($GroupIds) {
    ...
    if ($plans.Count -eq 0) {
        Write-PlannerLog "Keine Pläne gefunden (Zugriffsrechte prüfen, siehe Hinweise oben)." "ERROR"
        exit 1
    }
}
elseif ($GroupNames) {
...
elseif ($Interactive) {
    ...
    Write-Host "Möchten Sie aus den verfügbaren Gruppen auswählen? (J = Auswahl / N = Alle exportieren)" -ForegroundColor Yellow
    $choice = Read-Host
    ...
    if ($choice -eq "J" -or $choice -eq "j" -or $choice -eq "Y" -or $choice -eq "y") {
        ...
        Write-PlannerLog "Exportiere alle verfügbaren Gruppen ($($allGroups.Count) Gruppen)..."
        $plans = Get-PlansByGroupIds -GroupIds $allGroups.id
        if ($plans.Count -eq 0) {
            Write-PlannerLog "Keine Pläne gefunden (Zugriffsrechte prüfen, siehe Hinweise oben)." "ERROR"
            exit 1
        }
...
$exportSummary = @()
$script:exportedFileNames = @{}   # Dateinamen-Tracking für diesen Lauf (Duplikat-Erkennung)
